        ...
        if (Math.abs(level) < maxHops) {
            ...
            for (let parent_class_name in window.gUnityClasses) {
                ...
                for (let parent_field_name in parent_fields) {
                    ...
                    if (parent_field_type.replace(/\[\]$/, '') === class_name) {
...
function refreshStaticDetail() {
    ...
    function displayFieldDetail() {
        ...
        for (let i = 0; i < keys.length; i++) {
			...
			if (window.gUnityClasses[displayKlass][keys[i]].fieldClassFullName.replace(/\./g, "$")==displayKlass) 
				row += '<td>' + window.gUnityClasses[displayKlass][keys[i]].enumStr + ' - ' + keys[i] + '</td>'
			else 
            	row += '<td>' + keys[i] + '</td>'
...
function showunitypluginclassdetailpage(selectedClass) {
    ...
    $("#unitypluginclassdetailpage").find("button#download").unbind("click").click(function () {
        downloadClassDetail()
    });
...
function refreshOnSceneObjectList() {
    ...
    function extractObjFromFields(SourceObj, SourceObjKlass, TargetObjKlass, pathAry) {
        ...
        for (let level2Key in SourceKlassInfo) {
            ...
            let tmpFieldClassName = SourceKlassInfo[level2Key]["fieldClassName"];
            if (tmpFieldClassName)
                tmpFieldClassName = tmpFieldClassName.replace(/\[\]$/, '')
            if (SourceKlassInfo.hasOwnProperty(level2Key) && tmpFieldClassName == TargetObjKlass.split("$").pop()) {//Leve2Key is info/field name/method name
                ...
                if (SourceKlassInfo[level2Key]["fieldGenericType"] == "")
                    if (SourceKlassInfo[level2Key]["fieldClassName"].indexOf('[]') != -1) {//it is an Array
                        //debugInfo("TMPDEBUG Array TYPE: " + SourceObjKlass+ ":"+ level2Key+":"+SourceKlassInfo[level2Key]["fieldClassName"] , [SourceKlassInfo])

                        let objArray = new UnityArray(tmpAddr);

                        for (let i = 0; i < objArray.size(); i++) {
                            tmpAddr = objArray.get_Item(i);
                    ...
                    if (SourceKlassInfo[level2Key]["fieldGenericType"].indexOf("List") != -1) {
                        ...
                        if (targets.length == 0) continue; //no pointer exists
                    } else if (SourceKlassInfo[level2Key]["fieldGenericType"].indexOf("Dictionary") != -1) {
                        let objDictionary = new UnityDictionary(tmpAddr);

                        for (let i = 0; i < objDictionary.size(); i++) {
                            tmpAddr = objDictionary.get_Item(i);
                            if (tmpAddr == 0x0) continue; //no real pointer exists, it could be a dictionary storing list of empty items
                            targets.push("0x" + tmpAddr.toString(16))
                        }
                        if (targets.length == 0) continue; //no pointer exists
...
function downloadClassDetail(){
    ...
    let result = downloadTable()
    alert("Download for " + displayKlass + " on "+displayType + " is " + (result ? "successful" : "fail"))

    function downloadTable(){
        let content = "";
        let row = "";
		let objectDetailRow = "";
        $("#unitystaticdetailinfolist tbody tr").each(function(rowIndex) {
            $(this).find('td').each(function(colIndex) {
                var cellValue = $(this).text().replace(/\r?\n/g, "");
                var containsLink = $(this).has('a').length > 0; //indicate there are object link, which we want to parse the object content
                //console.log(`Value: ${cellValue}, Row: ${rowIndex + 1}, Column: ${colIndex + 1}`);
				//if (colIndex==0) row="";
				if (colIndex==1 && containsLink) {
					let objects = cellValue.split(",")
                    for (let i=0;i<objects.length;i++){
						let unityObjInfo = get_UnityObjInfo(Number(objects[i]),true)
						if (unityObjInfo.length ==0){
							objectDetailRow = row + "," + "Cannot get Object data for " + objects[i];
							...
							for (let j = 0; j < unityObjInfo.fieldDetails.length; j++) {
								if (i==0) header = (header=="" ? "object_address|"+unityObjInfo.fieldDetails[j].fieldName : header + "," + unityObjInfo.fieldDetails[j].fieldName)
								objectDetailRow = (j==0 ? objects[i]+"|"+unityObjInfo.fieldDetails[j].fieldValue : objectDetailRow + "," + unityObjInfo.fieldDetails[j].fieldValue)
							}
							if (i==0) content = content + row + "," + header + "\n";
							if (i<objects.length-1) content = content + row + "," + objectDetailRow + "\n";
							else row = row + "," + objectDetailRow;
						}
                        //objectDetailRow = "" //DO Something to get the object detail content in comma separated format
                        //objectDetailRow = row
                    }
				} else
                	row = (row=="" ? cellValue : row + "," + cellValue);
            });
            content = content + row + "\n";
            row = "";
        });
		
		let result= script.call("writeDataToFile",[displayKlass + "-"+displayType+".txt", content])
